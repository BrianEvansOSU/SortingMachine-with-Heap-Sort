...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (!isHeap(array, top, last, order)) { //***She said this wasn't needed
            int lt = 2 * top + 1;
            int rt = lt + 1;
            if (rt <= last) { //stops when end of heap is reached
                if (order.compare(array.entry(lt), array.entry(rt)) < 0) {
                    array.exchangeEntries(top, lt);//exchange top entry with lt entry if lt entry is greater than rt entry?
                    siftDown(array, lt, last, order);
                } else {
                    array.exchangeEntries(top, rt); //if lt entry isn't greater than rt entry, exchange top entry with rt entry?
                    siftDown(array, rt, last, order);
                }
            } else if (order.compare(array.entry(top), array.entry(lt)) < 0) {//***I changed the else to an else if. She said the else needed to check whether you needed to exchange entries, because you won't always need to here.
                //if end of array is reached, chek if left entry needs to be swapped?
                array.exchangeEntries(top, lt);
                siftDown(array, lt, last, order);
            }
        }
    }
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        int rt = 2 * top + 2;
        if (top < array.length()) {
            heapify(array, lt, order);
            heapify(array, rt, order);
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        Array<T> heapNew = new Array1L<T>(q.length());
        for (int pos = 0; q.length() > 0; pos++) {
            heapNew.setEntry(pos, q.dequeue());
        }
        heapify(heapNew, 0, order);
    ...
    public final T removeFirst() {
        ...
        T temp = this.heap.replaceEntry(0,
                this.heap.entry(this.heap.length() - 1)); //how do I remove an entry from this.heap?
        //I replaced the first entry with the last, but don't know how to remove the last entry from its initial position
        siftDown(this.heap, 0, this.heap.length() - 1, this.machineOrder);
    ...
    public final int size() {
        ...
        // TODO - fill in body
        //check mode and return size of heap or queue
